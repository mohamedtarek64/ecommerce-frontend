...
class ApiService {
  ...
  getCurrentUserId() {
    ...
    if (!userId) {
      ...
      if (storedUserId) {
        ...
        console.log('💾 User ID from localStorage user_id:', userId)
      }
    }
      ...
      if (userData) {
        try {
          ...
          if (user && (user.id || user.user?.id)) {
            ...
            console.log('👤 User ID from localStorage user_data:', userId)
          }
        } catch (e) {
    ...
    if (!userId) {
      console.log('⚠️ No user authentication found, using default user 35 for testing')
      userId = 35
    }
  ...
  async request(endpoint, options = {}) {
    ...
    try {
      console.log(`🚀 API Request: ${finalOptions.method || 'GET'} ${finalUrl}`)
      if (options.body) {
  ...
  async addToCart(productId, quantity = 1, color = '', size = '', productTable = 'products_women') {
    ...
    console.log('📦 API Service addToCart called with:', {
      productId,
      quantity,
      color,
      size,
    ...
    return this.post('/cart/add', {
      ...
      user_id: parseInt(localStorage.getItem('user_id')) || 18
    })
  }

  async updateCartItem(itemId, quantity) {
    const userId = parseInt(localStorage.getItem('user_id')) || 18
    console.log(`🌐 API: Updating cart item ${itemId} with quantity ${quantity} for user ${userId}`)
