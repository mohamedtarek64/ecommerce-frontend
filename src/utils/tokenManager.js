...
export class TokenManager {
  ...
  static setUser(user) {
    if (user) {
      ...
      console.log('✅ TokenManager.setUser() called:', user.role)
    }
  }
  ...
  static isAuthenticated() {
    ...
    // Only log in development and reduce frequency
    if (process.env.NODE_ENV === 'development' && Math.random() < 0.1) {
      console.log('🔍 TokenManager.isAuthenticated():', {
        hasToken,
        isExpired,
        result,
  ...
  static getUserRole() {
    ...
    // Only log in development and reduce frequency
    if (process.env.NODE_ENV === 'development' && Math.random() < 0.1) {
      console.log('🔍 TokenManager.getUserRole():', {
        user: user,
        payload: payload,
        role: role
      })
  ...
  static isAdmin() {
    ...
    // Only log in development and reduce frequency
    if (process.env.NODE_ENV === 'development' && Math.random() < 0.1) {
      console.log('🔍 TokenManager.isAdmin() called:', result)
    }
    return result
  }
