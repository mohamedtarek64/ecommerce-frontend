...
export const useCartStore = defineStore('cart', () => {
        ...
        if (itemData.color_name || itemData.color) {
          ...
          if (savedProductData) {
            try {
              ...
              console.log('Error parsing saved product data:', e)
            }
          }
        }
  ...
  const addToCart = async (product, selectedSize = null, selectedColor = null, quantity = 1) => {
    ...
    if (newItem.productData.colors.length > 0) {
      ...
      localStorage.setItem(`cart_product_${product.id}`, JSON.stringify(cartItemData))
    }
    ...
    try {
      ...
      const userId = localStorage.getItem('user_id') || 'not_found'

      console.log('🛒 Cart store using product table:', productTable, 'for product:', product.id, 'user_id:', userId)
