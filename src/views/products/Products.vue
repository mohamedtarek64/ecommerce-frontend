  ...
  <div class="flex min-h-screen flex-col bg-[#0f0a06] text-white" style='font-family: "Space Grotesk", "Noto Sans", sans-serif;'>
    ...
    <main class="container mx-auto flex-1 px-4 py-6 lg:px-10 relative">
      ...
      <div class="grid grid-cols-1 gap-8 lg:grid-cols-4">
        ...
        <aside class="filter-section space-y-6 lg:col-span-1 p-6 rounded-2xl">
          ...
          <div class="space-y-6">
            ...
            <div class="space-y-3">
              ...
              <select
                ...
                class="form-select w-full rounded-xl py-3 px-4"
              >
            ...
            <div class="space-y-3">
              <label class="flex items-center gap-2">
                ...
                <span class="text-sm font-semibold text-gray-300">Size</span>
              </label>
              <select
            ...
            <div class="space-y-3">
              ...
              />
                <div class="flex justify-between text-sm">
                  ...
                  <span class="gradient-text font-semibold">${{ filters.price || 500 }}+</span>
                </div>
              </div>
              </div>
            ...
            <div class="space-y-3">
              <label class="flex items-center gap-2">
                <span class="material-symbols-outlined text-gray-400">palette</span>
                <span class="text-sm font-semibold text-gray-300">Colors</span>
              </label>
              <div class="flex flex-wrap gap-3">
                <button
                  ...
                  :class="[
                    'color-filter-btn size-10 rounded-full transition-all duration-300',
          ...
          <div class="space-y-3 pt-4 border-t border-gray-700">
            <button
              @click="applyFilters"
              class="apply-filters-btn w-full rounded-xl py-4 font-bold text-white text-lg"
            >
            ...
            <button
              @click="clearFilters"
              class="clear-filters-btn w-full rounded-xl py-3 font-semibold transition-all duration-300"
            >
            ...
            >
              ...
              <div class="aspect-square w-full overflow-hidden rounded-xl bg-gradient-to-br from-[#0a0a0a] to-[#1a1a1a] cursor-pointer relative" @click="viewProduct(product.id)">
                ...
                <div class="product-overlay rounded-xl">
                  ...
                  <button
                      @click.stop="addToCartEnhanced(product.id)"
                      class="product-action-btn"
                      :class="{ 'loading': buttonLoadingStates[`cart-${product.id}`] }"
                      :disabled="buttonLoadingStates[`cart-${product.id}`]"
                      title="Add to Cart"
                  >
                      <span
                        v-if="!buttonLoadingStates[`cart-${product.id}`]"
                    ...
                    <button
                      ...
                      class="product-action-btn"
                      :class="{ 'loading': buttonLoadingStates[product.id] }"
                      :disabled="buttonLoadingStates[product.id]"
                      :title="wishlistItemIds.has(product.id) ? 'Remove from Wishlist' : 'Add to Wishlist'"
                    >
                      <span
                        v-if="!buttonLoadingStates[product.id]"
                        class="material-symbols-outlined text-xl"
                        :class="{ 'text-red-500': wishlistItemIds.has(product.id) }"
                      >
                        {{ wishlistItemIds.has(product.id) ? 'favorite' : 'favorite_border' }}
                      </span>
                      <span
                        v-else
                        class="material-symbols-outlined text-xl animate-spin"
...
const handleWatchCollection = () => {
  ...
  console.log('Watch collection video')
