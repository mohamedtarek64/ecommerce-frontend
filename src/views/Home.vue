...
onMounted(async () => {
  ...
  performanceMonitor.startTiming('home_page_load')

  // Phase 1: Critical initialization (run first)
  performanceMonitor.startTiming('home_critical_init')
  ...
  performanceMonitor.endTiming('home_critical_init')

  // Phase 2: Prioritized data loading (critical first, then parallel)
  performanceMonitor.startTiming('home_data_loading')

  // Step 1: Load critical above-the-fold content first
  performanceMonitor.startTiming('home_critical_data')
  const criticalTasks = [
    loadFeaturedProducts(), // Featured section is above the fold
  ]
  ...
  if (isAuthenticated && user) {
    ...
    if (token) {
      criticalTasks.push(
        fetchWelcomeMessage().catch(() => { /* keep UI intact */ })
      )
    }
  }
  ...
  performanceMonitor.endTiming('home_critical_data')

  // Step 2: Load remaining content in parallel (below the fold)
  performanceMonitor.startTiming('home_secondary_data')
  const secondaryTasks = [
    loadAdidasProducts(),
    loadMenProducts(featuredImages.value, categoryImages.value, instagramImages.value),
    loadWomenProducts(featuredImages.value, categoryImages.value, instagramImages.value),
    loadKidsProducts(featuredImages.value, categoryImages.value, instagramImages.value).then(() => {
      console.log('🔍 Kids products after loading:', kidsProducts.value.length, 'products')
  ...
  // Don't wait for secondary content (will load in background)
  Promise.all(secondaryTasks).then(() => {
    performanceMonitor.endTiming('home_secondary_data')
  })

  performanceMonitor.endTiming('home_data_loading')
  ...
  const pageLoadTime = performanceMonitor.endTiming('home_page_load')
  if (pageLoadTime) {
    performanceMonitor.recordMetric('home_page_load_time', pageLoadTime, {
      authenticated: isAuthenticated.value,
      hasUser: !!user.value
    })
    console.log(`🚀 Home page loaded in ${pageLoadTime.toFixed(2)}ms`)
  }
})
