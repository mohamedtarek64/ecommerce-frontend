  ...
  try {
    loading.value = true
      ...
      const orderData = {
        items: cartItems.value.map(item => ({
          id: item.product_id || item.id, // Use product_id first, fallback to cart item id
      ...
      if (paymentFormRef.value) {
        try {
          ...
          console.warn('Payment failed, using demo mode:', paymentError)
          // For demo purposes, simulate successful payment
          alert('Payment processed in demo mode (Stripe connection issue)')
    ...
    if (result.success) {
      ...
      // Clear cart
          ...
          router.push(`/order-success/${result.data.order_number}`)
    } else {
  ...
  if (newShipping) {
    ...
    if (method) {
      ...
      console.log('Shipping method selected:', {
        ...
        tax: taxAmount.value,
        total: currentSubtotal + currentShipping + taxAmount.value
      })
    }
  }
})
...
onMounted(async () => {
  ...
  console.log('Checkout initialized with cart data:', {
    ...
    appliedDiscountCode: appliedDiscountCode.value
  })

  console.log('Shipping methods:', {
    shippingMethods: shippingMethods,
    value: shippingMethods.value,
    isRef: typeof shippingMethods === 'object' && 'value' in shippingMethods,
    isArray: Array.isArray(shippingMethods) || Array.isArray(shippingMethods.value)
  })
