...
const error = computed(() => {
  ...
  if (errorValue) {
    console.log('🔍 Current error value:', errorValue)
  }
  return errorValue
})
...
const showErrorState = computed(() => {
  ...
  if (!error.value) {
    console.log('🔍 No error value, hiding error state')
    return false
  }
  ...
  try {
    ...
    console.log('🔍 Error converting to string, hiding error state')
    return false
  }

  if (!errorMessage) {
    console.log('🔍 Empty error message, hiding error state')
    return false
  }
  ...
  console.log('🔍 Should show error state:', shouldShow, 'isIgnoredError:', isIgnoredError)
...
const initializeStripe = async () => {
  try {
    ...
    if (stripe.value && checkoutPayment.stripeInitialized.value) {
      console.log('Stripe already loaded and initialized, skipping...')
      return
    }
...
const initializePayment = async () => {
  ...
  if (checkoutPayment.loading.value || checkoutPayment.stripeInitialized.value) {
    console.log('Payment initialization already in progress or completed, skipping...')
    return
  }
...
onMounted(async () => {
  console.log('PaymentForm mounted, auto-initializing Stripe...')
...
onMounted(async () => {
  console.log('🎯 PaymentForm mounted, selected method:', selectedMethod.value)
