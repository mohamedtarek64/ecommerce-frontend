...
export function useOrdersAPI(isLoading, error, orders) {
  ...
  const performDuplicate = async (order) => {
    try {
      ...
      // For now, we'll just show a message that duplication is not implemented
      // In a real application, you would create a new order with the same items but different ID
      const confirmed = confirm(`📋 Duplicate Order #${order.id}?\n\nCustomer: ${order.customer_name || 'N/A'}\nTotal: ${formatCurrency(order.total_amount)}\nItems: ${order.total_items || 0} items\n\nThis will create a copy of this order with a new order number.`)
      ...
      if (confirmed) {
        ...
        duplicateInfo += `Customer: ${order.customer_name || 'N/A'}\n`
        duplicateInfo += `Total: ${formatCurrency(order.total_amount)}\n`
        duplicateInfo += `Status: pending (reset for new order)\n`
        duplicateInfo += `Items: ${order.total_items || 0} items\n`
        ...
        alert(`✅ Order #${order.id} duplication preview completed!\n\n(In production, this would create a new order)`)
      }
    } catch (err) {
...
export function useOrdersActions(showMoreActions, showConfirmModal, confirmModal, showToast, toastMessage, toastType, statusFilter, searchQuery, performDelete, performDuplicate) {
  const applyFilters = () => {
