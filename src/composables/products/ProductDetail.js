...
export const useProductDetailComputed = (product, selectedImage) => {
  ...
  const availableColors = computed(() => {
    if (!product.value) {
      console.log('🔍 availableColors: No product loaded')
      return []
    }
    const colors = product.value.colors || []
    console.log('🔍 availableColors computed:', {
      productId: product.value.id,
      productName: product.value.name,
      colorsCount: colors.length,
      colors: colors.map(c => ({ id: c.id, name: c.color_name, code: c.color_code })),
      rawColors: colors
    })
    return colors
  })
...
export const useProductDetailAPI = () => {
  ...
  const loadProduct = async (productId, loading, error, product, options = {}) => {
    console.log('🚀 Loading product with ID:', productId, options)
    loading.value = true
    error.value = ''
    ...
    try {
      ...
      if (options.category) {
        ...
        console.log('📂 Loading from category:', options.category)
      } else if (options.table) {
        params.append('table', options.table)
        console.log('📂 Loading from table:', options.table)
      }
      ...
      if (data.success) {
        ...
        console.log('✅ Product loaded successfully:', {
          ...
          colorsCount: product.value.colors ? product.value.colors.length : 0,
          colors: product.value.colors ? product.value.colors.map(c => ({ id: c.id, name: c.color_name, code: c.color_code })) : []
        })
  ...
  const loadProductColors = async (productId, product) => {
    try {
      ...
      if (product.value.colors && product.value.colors.length > 0) {
        console.log('✅ Using colors from main API:', product.value.colors)
        return
      }
      ...
      if (response.ok) {
        ...
        if (data.success && data.data) {
          ...
          console.log('Product colors loaded from separate API:', data.data)
        }
      } else {
  ...
  const addToCart = async (productId, quantity, selectedSize, selectedColor, isAddingToCart, success, error) => {
    ...
    try {
      ...
      console.log('=== ADD TO CART API FUNCTION COMPLETED ===')
    }
  }
  ...
  const optimizedLoadCart = async (userId) => {
    try {
      ...
      const data = await response.json()
      console.log('=== OPTIMIZED LOAD CART COMPLETED ===')
      return data
    } catch (error) {
  ...
  const loadWishlistCount = async () => {
    try {
      ...
      const data = await response.json()
      if (data.success) {
        console.log('✅ loadWishlistCount - Count:', data.data?.count)
...
export const useProductDetailHandlers = (
  ...
  const selectSize = (size) => {
    console.log('=== SELECT SIZE CALLED ===')
    console.log('Selected size:', size)
    console.log('Previous selectedSize:', selectedSize.value)
    selectedSize.value = size
    console.log('New selectedSize:', selectedSize.value)
    console.log('=== SELECT SIZE COMPLETED ===')
  }

  const selectColor = (color) => {
    console.log('=== SELECT COLOR CALLED ===')
    console.log('Selected color:', color)
    console.log('Previous selectedColor:', selectedColor.value)
    selectedColor.value = color
    console.log('New selectedColor:', selectedColor.value)
    console.log('=== SELECT COLOR COMPLETED ===')
  }
  ...
  const handleAddToCart = async () => {
    ...
    console.log('Add to cart clicked:', {
      ...
      selectedColorType: typeof selectedColor.value,
      selectedColorValue: selectedColor.value?.color_name || selectedColor.value
    })
    ...
    console.log('=== HANDLE ADD TO CART COMPLETED ===')
  }
...
export const initializeProductDetail = (
  ...
  onMounted(async () => {
    ...
    console.log('🎯 ProductDetail onMounted - Product ID:', productId)
    if (productId) {
  ...
  watch(() => route.params.id, async (newId, oldId) => {
    console.log('🔄 ProductDetail route watcher - Old ID:', oldId, 'New ID:', newId)
    if (newId && newId !== oldId) {
      console.log('📦 Loading new product:', newId)
