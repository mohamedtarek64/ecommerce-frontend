...
export function useCart() {
  ...
  // Cart summary for Cart.vue
  const subtotal = computed(() => totalPrice.value || 0)
  const shippingCost = computed(() => subtotal.value > 100 ? 0 : 10) // Free shipping over $100
  const taxAmount = computed(() => subtotal.value * 0.08) // 8% tax on subtotal
  const totalAmount = computed(() => subtotal.value + shippingCost.value + taxAmount.value - discountAmount.value)
  ...
  const loadCartFromStorage = () => {
    ...
    if (backup) {
      try {
        ...
        console.log('Cart backup found but using store instead')
      } catch (err) {
  ...
  const applyPromoCode = async () => {
    ...
    try {
      ...
      console.log('Applying promo code:', requestData)
      console.log('Request data types:', {
        code: typeof requestData.code,
        total_amount: typeof requestData.total_amount,
        product_ids: Array.isArray(requestData.product_ids),
      ...
      base.showError(error.message || 'Failed to apply promo code')
