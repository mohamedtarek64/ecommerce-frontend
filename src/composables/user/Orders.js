...
export function useOrdersAPI(isLoading, showError) {
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          if (result.data && Array.isArray(result.data.data)) {
            ...
            console.log('✅ Orders loaded from paginated response:', orders.value.length, 'orders')
          } else if (Array.isArray(result.data)) {
            orders.value = result.data
            console.log('✅ Orders loaded from array response:', orders.value.length, 'orders')
          } else {
            orders.value = []
          ...
          console.log('Orders data structure:', result.data)
        } else {
  ...
  const cancelOrder = async (orderToCancel, orders, showCancelModal, selectedOrder, success, showError) => {
    ...
    try {
      console.log('Cancelling order:', orderToCancel.value.id)
